...
| **Phase 2** | **Lab Module (Bilingual EN/ES)**<br>• **Lab Admin Process Flow & User Management (`/lab/users/admin`)**: Dedicated administration screen for Dental Lab managers and supervisors. Strictly restricted to Tenant Administrators for creation and branch assignments; enforces branch prerequisite (at least one branch required before creation); screenshot-accurate Create Admin modal with First Name, Last Name, Email, Country Code + Phone, Branch selection via `<SearchableSelect />`, and Default Lab Admin flag; automatically dispatches localized welcome email with a 24-hour single-use reset password link for instant activation.<br>• Prosthesis & Process master data<br>• Work Orders & Process State Machine<br>• Internal & External Verifications (email alerts)<br>• QR Public Tracking & Interest Collection<br>• Deliveries & Lab Dashboard | 🟡 *In Progress* |
